...
## What is actually being compared (read before quoting any number)

The two sides do **not** receive the same prompt, by design:

| | prompt it receives | how it learned the task |
|---|---|---|
| Groq (Llama-3.3-70B class) | `SYSTEM_PROMPT` + **3 few-shot examples** + posting (~2156 tokens) | in-context only, never trained |
| Fine-tuned Qwen2.5-1.5B | `SYSTEM_PROMPT` + posting (~1570 tokens) | QLoRA on 541 labelled postings |

Groq gets ~586 tokens of worked examples the fine-tuned model never sees
(`label/prompt.py::build_messages` injects `_FEWSHOT_EXAMPLES`; the benchmark sends the
fine-tuned model `[system, user]` only). Both sides use the **same** `SYSTEM_PROMPT`
object, so the schema/rules text cannot drift between them.

This is a **system-level** comparison -- "fine-tuned small model vs few-shot-prompted large
model", each in its intended configuration -- which is the claim the README actually makes.
It is **not** a controlled prompt ablation, and it should not be described as one. The
fine-tuned model's prompt is also identical to what it saw in training, which is the
property that matters for it.

A second asymmetry worth stating: the labels the fine-tuned model learned from were
*generated by Groq*. This is distillation, so Groq's systematic errors are partly inherited
(`LABELLING-NOTES.md` documents the cleanup passes that removed the mechanically-detectable
ones). A student trained on a teacher's output does not normally exceed the teacher on the
teacher's own failure modes -- worth remembering when reading the `required_skills` row,
where Groq itself only scores F1 0.565.
